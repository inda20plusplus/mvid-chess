...
fn test_free_movement() {
    ...
    let possible_moves = board.get_allowed_moves(&Point(3, 2)).unwrap();
...
fn test_movement_blocked_by_same_color() {
    ...
    let possible_moves = board.get_allowed_moves(&Point(3, 3)).unwrap();
...
fn test_completely_blocked() {
    ...
    assert!(board.get_allowed_moves(&Point(3, 3)).is_none());
}
...
fn test_pawn_normal_movement() {
    ...
    assert_eq!(
        board.get_allowed_moves(&Point(1, 3)).unwrap(),
        vec![Point(1, 4)]
    );
    assert_eq!(
        board.get_allowed_moves(&Point(8, 6)).unwrap(),
        vec![Point(8, 5)]
    );
}
...
fn test_pawn_capture() {
    ...
    assert_eq!(
        board.get_allowed_moves(&Point(3, 3)).unwrap().as_sorted(),
        vec![Point(4, 4), Point(2, 4), Point(3, 4)].as_sorted()
    );
    assert_eq!(
        board.get_allowed_moves(&Point(4, 4)).unwrap().as_sorted(),
...
fn test_pawn_has_not_moved() {
    ...
    assert_eq!(
        board.get_allowed_moves(&Point(1, 2)).unwrap().as_sorted(),
        vec![Point(1, 3), Point(1, 4)].as_sorted()
    );

    assert_eq!(
        board.get_allowed_moves(&Point(1, 7)).unwrap().as_sorted(),
        vec![Point(1, 6), Point(1, 5)].as_sorted()
    );

    assert!(board.get_allowed_moves(&Point(2, 7)).is_none());
}
...
fn test_cannot_unblock_king() {
    ...
    assert_eq!(
        board.get_allowed_moves(&Point(5, 3)).unwrap().as_sorted(),
    ...
    assert_eq!(
        board.get_allowed_moves(&Point(4, 2)).unwrap().as_sorted(),
        vec![Point(4, 3), Point(4, 4)].as_sorted()
    );

    assert!(board.get_allowed_moves(&Point(6, 2)).is_none());
}
...
fn test_piece_must_protect_king_when_checked() {
    ...
    assert_eq!(
        board.get_allowed_moves(&Point(5, 6)).unwrap(),
        vec![Point(5, 4)]
    );

    assert_eq!(
        board.get_allowed_moves(&Point(6, 6)).unwrap().as_sorted(),
        vec![Point(4, 4), Point(8, 4)].as_sorted()
    );

    assert_eq!(
        board.get_allowed_moves(&Point(7, 2)).unwrap().as_sorted(),
